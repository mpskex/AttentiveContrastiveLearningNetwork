"""
Stanford-dogs/stanford_dogs_data.py at master · zrsmithson/Stanford-dogs
...
class dogs(data.Dataset):
    ...
    def _get_label(self, idx):
        _, target_class = self._flat_breed_images[indx]
        if self.target_transform:
            target_class = self.target_transform(target_class)
        return target_class
...
@REGISTER_DATALOADER('Stanford_Dogs')
def build_Stanford_Dogs(param, split='train', tta=False):
    import inspect
    from os.path import dirname, join
    root = join(dirname(dirname(dirname(inspect.getabsfile(dogs)))), param.data_root)
    if split == 'train':
        _train = True
        ...
        _train = True
        transform = get_transform(True, tta, rot=30)
    return dogs(root=root, train=_train, download=True, transform=transform)
...
if __name__ == '__main__':
    ...
    dataset = build_Stanford_Dogs(cfg(), split='train')
